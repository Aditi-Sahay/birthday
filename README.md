...
If reusing this as a portfolio piece, worth calling out in a case study:
- The concept tie-in between the runaway "no" button and the self-love question it's attached to
- The screen-by-screen state machine handled with plain JS (no framework overhead for a 4-screen flow)
- CSS-only pixel/CRT effects (scanlines, glitch text, pixel-notched card corners) built without images or a canvas
